...
VALUES 
    ...
    ('Acyclovir', '', 45.0),
    ('Allopurinol', '', 30.8),
    ('Ibuprofen', 'Anti-inflammatory medication', 19.3),
    ('Paracetamol', 'Painkiller and fever reducer', 12.0),
    ('Diclofenac', 'Anti-inflammatory pain reliever', 29.8);

GO

INSERT INTO "Order"(IdProduct, Amount, CreatedAt)
VALUES
    ((SELECT IdProduct FROM Product WHERE Name='Abacavir'), 125, GETDATE()),
    ((SELECT IdProduct FROM Product WHERE Name='Ibuprofen'), 120, GETDATE()),
    ((SELECT IdProduct FROM Product WHERE Name='Paracetamol'), 250, GETDATE()),
    ((SELECT IdProduct FROM Product WHERE Name='Diclofenac'), 180, GETDATE());